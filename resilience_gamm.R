...
#varying curve 
div_vsm_bam<-bam(wbeing_div ~ s(year_div, k = 48) + s(year_div, subj_wid, bs = "fs"), data=div)
wid_vsm_bam<-bam(wbeing_wid ~ s(year_wid, k = 48) + s(year_wid, subj_wid, bs = "fs"), data=wid)
emp_vsm_bam<-bam(wbeing_emp ~ s(year_emp, k = 48) + s(year_emp, subj_wid, bs = "fs"), data=empred)
